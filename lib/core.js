...
// Estado por usuário
const S = new Map(); // from -> { stage, draft: {produto, quantidade, unidade, marca}, pedidoId, lastMsgTs, shortBurst }
...
function getState(id){
  if (!S.has(id)) S.set(id, { stage: 'initial', draft: {}, pedidoId: null, lastMsgTs: 0, shortBurst: 0 });
  return S.get(id);
}
...
export async function onText({ from, text, send }){
  ...
  if (state.stage === 'initial'){
    await send('Sou da CompraForte. Eu busco preços e te retorno 2–3 ofertas com frete. Tem comércio de alimentação?');
    state.stage = 'intro';
    return;
  }
  ...
  if (state.stage !== 'awaiting_neighborhood' && miss.length === 0){
    ...
    await send(`✅ Pedido ${pedidoId} registrado: ${resumo}\nQual bairro para calcular frete/prazo?`);
  ...
  if (state.stage === 'awaiting_neighborhood'){
    ...
    if (bairro.length < 2){
      await send('Qual o **bairro**? Ex.: Ponta Verde, Jatiúca, Centro.');
      return;
    }
    try{
    ...
    await send(`Bairro registrado: ${bairro}. Vou buscar 2–3 ofertas e retorno ainda hoje.`);
  ...
  // 6) Fallback (raro)
  await send('Vamos começar: qual **produto** você quer cotar? Depois me diga **quantidade + unidade** (ex.: 10kg, 5 cx).');
