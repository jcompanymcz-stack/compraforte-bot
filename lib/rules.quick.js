// lib/rules.quick.js (FIX) — evita regex inválida com caracteres especiais como "?"
...
const cobranca = ['quanto custa','tem mensalidade','vocês cobram','vocês cobram?','pagar','preco do servico','taxa do servico','eh de graca','é de graça','paga quanto'];
...
// Presença com "?" precisa de regex específica (não usar unir com texto cru)
const presenceRegexes = [
  /^\?{1,3}$/i,                // "?", "??", "???"
  /^oi\?$/i, /^al[oô]\?$/i, /^alo\?$/i, /^alô\?$/i,
...
export const rules = [
  ...
  // pedido direto (heurística forte - números + unidades)
